...
public class AuctionApiClient
{
    ...
    public async Task<List<AuctionResultDto>> GetAuctionResultsByBrokerAsync(int brokerId, int? auctionId = null)
        => await _http.GetFromJsonAsync<List<AuctionResultDto>>(
               auctionId is int aid
                   ? $"api/auction-results/broker/{brokerId}?auctionId={aid}"
                   : $"api/auction-results/broker/{brokerId}") ?? new();
    ...
    public async Task<List<AuctionResultDto>> GetAuctionResultsByBuyerAsync(int buyerId, int? auctionId = null)
        => await _http.GetFromJsonAsync<List<AuctionResultDto>>(
               auctionId is int aid
                   ? $"api/auction-results/buyer/{buyerId}?auctionId={aid}"
                   : $"api/auction-results/buyer/{buyerId}") ?? new();
    ...
    public async Task<BcSyncResultDto?> SyncFarmersAsync()
    {
        var resp = await _http.PostAsync("api/bc/sync/farmers", null);
        if (!resp.IsSuccessStatusCode) return null;
        return await resp.Content.ReadFromJsonAsync<BcSyncResultDto>();
    }
    ...
    public async Task<HttpResponseMessage> SimulateTypistEntriesAsync(int auctionId, int disagreementPercent, int? maxLots, decimal minPrice, decimal maxPrice, string disagreementType, int? brokerId)
        => await _http.PostAsJsonAsync("api/typist-entries/simulate", new { auctionId, disagreementPercent, maxLots, minPrice, maxPrice, disagreementType, brokerId });
    ...
    public async Task<bool> UpdatePackingOrderStatusAsync(int id, string status)
    {
        var resp = await _http.PutAsJsonAsync($"api/shipments/packing-orders/{id}/status", new { status });
        return resp.IsSuccessStatusCode;
    }
